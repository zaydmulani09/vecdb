...
This is exactly the "search then throw away results that don't match" anti-pattern the v2 plan calls out. At high filter selectivity (95%+ eliminated), HNSW returns `candidate_k` hits and most get discarded → recall collapses unless `candidate_k` is inflated massively. **Filtering is NOT fused into graph traversal / candidate generation.** Phase 3 is genuine work, not a relabel.
